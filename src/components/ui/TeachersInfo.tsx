...
const TeachersInfo: React.FC = () => {
  return (
    <div
      className="bg-[var(--bg)] flex flex-col justify-between h-screen p-8"
      style={{ fontFamily: "'Roboto', sans-serif" }}
    >
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-4xl font-bold text-[var(--first)]">Deen Mohammad</h1>
          <p className="text-lg text-[var(--first)]">Commerce</p>
          <p className="text-lg text-[var(--first)]">
            11<sup>th</sup> A
          </p>
        </div>
        <div>
          <p className="text-lg text-[var(--first)]">786</p>
        </div>
      </div>
        ...
        <div className="border-l-2 border-[var(--bg)] pl-4">
          {subjects.map((subject) => {
            ...
            return (
              <Link
                key={subject}
                href={`/dashboard/teachers/${formattedSubject}`}
              >
